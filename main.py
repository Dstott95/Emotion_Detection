...
def fit_z_score(df):
    # Set up lists to store the means and stds
    means = np.mean(df[:, 1:], axis=1)
    stds = np.std(df[:, 1:], axis=1)
    return means, stds


def z_score(df, means, stds):
    # copy the dataframe
    df_std = df.copy()
    # apply the z-score normalization
    for column in range(1, N_INPUTS):
        df_std[:, column] = \
            (df_std[:, column] - means[column]) / stds[column]
    return df_std
...
class EmotionDataset(Dataset):
    ...
    def __getitem__(self, index):
        # i = index*self.sequence_length
        label = int(self.data[index, 0, 0]-1)
        # label = int(self.data.iloc[i,0])
        features = self.data[index, :, 1:]
        # features = np.asarray(self.data.iloc[i:i+self.sequence_length,1:], dtype=np.float)
...
# Shapes of tensors
# input -> batch_size, seq_length, features (input_size)
# -> batch, 290, 64
# h_0 -> n_layers, batch_size, hidden_dim
# -> 2, batch, 128
# c_0 -> n_layers, batch_size, hidden_dim
# -> 2, batch, 128
...
# -> 2, batch, 128
    ...
    # Load datasets
    train_dataset = EmotionDataset(r"C:\\Users\\darre\\PycharmProjects\\Emotion_Classifier\\audio\\norm_train_data.csv", sequence_length=N_SEQUENCE)
    train_dataset.reshape()

    test_dataset = EmotionDataset(r"C:\\Users\\darre\\PycharmProjects\\Emotion_Classifier\\audio\\norm_test_data.csv", sequence_length=N_SEQUENCE)
...
'''if __name__ == '__main__':
    ...
    lstm = nn.LSTM(n_inputs, 2)  # Input dim is input_shape, output dim is 2

    model = LSTM(N_INPUTS, HIDDEN_DIM, len(samples), len(features))
